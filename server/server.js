...
import serviceAccountKey from "./react-js-blog-website-19362-firebase-adminsdk-fbsvc-28cc6bd4e5.json" assert {type: "json"}
...
server.post("/google-auth", async (req, res) => {
    ...
    .catch(err => {
        return res.status(500).json({"error": "Failed to authenticate you with google. Try to with some other google account"})
    })
})
...
server.post("/change-password", verifyJWT, (req, res) => {
    ...
    .then((user) => {
        ...
        bcrypt.compare(currentPassword, user.personal_info.password, (err, result) => {
            if(err){
                return res.status(500).json({ error: "Some error occured while changing the password, pleasa try again later"})
            }
...
server.post('/lastest-blogs', (req, res) => {
...
server.post('/all-lastest-blogs-count', (req, res) => {
...
server.post("/update-profile", verifyJWT, (req, res) => {
    ...
    if(username.length < 3){
        return res.status(403).json({ error: "Username shuold be at least 3 letters long"})
    }
...
server.post('/create-blog', verifyJWT,(req, res) => {
    ...
    if(!draft){
        ...
        if(!banner){
            return res.status(403).json({ error: "You must provide blog banner to publisht it"})
        }
...
server.post('/add-comment', verifyJWT,(req, res) => {
    ...
    new Comment(commentObj).save().then( async commentFile => {
        ...
        .then(blog => { console.log('New comment create') })
        ...
        new Notification(notificationObj).save().then(notification => console.log('new notification create'))
...
server.post('/get-blog-comments', (req, res) => {
    ...
    populate("commented_by", "personal_info.username personal_info.fullname personal_info.img")
    .skip(skip)
    .limit(maxLimit)
    .sort({
        'commentedAt': -1
...
const deleteComments = (_id) => {
    ...
    .then(comment => {
        ...
        Notification.findOneAndUpdate({ reply: _id }, {$unset: {reply: 1}}).then(notification => console.log('reply notifacation deleted'))
